"""Workflow status endpoints for external orchestration."""
...
from floorplan_service.crud.workflow_status_crud import (
    get_status,
    list_statuses,
    upsert_status,
)
from floorplan_service.db import get_db
from floorplan_service.schemas.workflow_status import (
...
async def list_workflow_statuses(
    ...
    """Return every context tracked for the provided super ID."""
...
async def read_workflow_status(
    ...
    """Return the latest status for a specific context."""
...
async def upsert_workflow_status_endpoint(
    ...
    """Create or update a workflow status record."""
    record = await upsert_status(
        db,
