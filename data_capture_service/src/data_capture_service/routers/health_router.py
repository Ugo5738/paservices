"""
Health check endpoints for monitoring Data Capture Service status.
"""
...
from data_capture_service.schemas.common import (
    ComponentHealth,
    HealthCheckResponse,
    HealthStatus,
)
...
async def detailed_health_check(
    ...
    # Include Firecrawl baseline provider status
    response_dict["components"]["firecrawl_baseline"] = {
        "status": HealthStatus.OK if settings.firecrawl_enabled() else HealthStatus.WARNING,
        "message": "Enabled" if settings.firecrawl_enabled() else "Disabled (no API key)",
    }

    # Include Motie adapter status
    response_dict["components"]["motie_adapter"] = {
        "status": HealthStatus.OK if settings.motie_enabled() else HealthStatus.WARNING,
        "message": "Enabled" if settings.motie_enabled() else "Disabled",
    }
