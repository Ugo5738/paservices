...
def parse_connection_url(url):
    ...
    pgbouncer_enabled = 'pgbouncer' in query_params and query_params['pgbouncer'] == 'true'
...
async def check_async_connection(url, env_name="unknown", verbose=False):
    ...
    clean_url_parts.append(f"{conn_params['host']}:{conn_params['port']}/{conn_params['dbname']}")
    ...
    if conn_params['pgbouncer_enabled']:
        ...
        connect_args["options"] = "-c standard_conforming_strings=on -c client_encoding=utf8 -c plan_cache_mode=force_custom_plan -c statement_timeout=0 -c default_transaction_isolation=read\ committed"
    ...
    try:
        ...
        async with engine.begin() as conn:
            # Test a simple query
            result = await conn.execute(text("SELECT 1 as test, current_timestamp, current_database(), current_user"))
            row = result.fetchone()
...
async def check_schemas():
    ...
    parser.add_argument('--env', choices=['dev', 'prod', 'test', 'all'], default='dev',
    ...
    default_url = settings.auth_service_database_url
