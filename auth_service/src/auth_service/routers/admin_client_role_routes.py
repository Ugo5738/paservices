...
async def assign_role_to_client(
    ...
    logger.info(f"Admin user attempting to assign role {role_assignment.role_id} to app client {client_id}")
    ...
    new_client_role = AppClientRole(app_client_id=client_id, role_id=role_assignment.role_id)
...
async def list_client_roles(
    ...
    query = select(AppClientRole).where(AppClientRole.app_client_id == client_id).options(selectinload(AppClientRole.role))
    ...
    return AppClientRoleListResponse(items=client_role_responses, count=len(client_role_responses))


@router.delete("/clients/{client_id}/roles/{role_id}", 
...
async def remove_role_from_client(
    ...
    logger.info(f"Admin user attempting to remove role {role_id} from app client {client_id}")
    ...
    query = select(AppClientRole).join(Role).where(
        and_(
            AppClientRole.app_client_id == client_id,
