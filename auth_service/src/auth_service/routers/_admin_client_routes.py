...
async def create_app_client(
    ...
    existing_client_stmt = select(AppClient).where(
        AppClient.client_name == client_data.client_name
    )
    ...
    if client_data.assigned_roles:
        for role_name in client_data.assigned_roles:
            ...
            if not role:
                logger.warning(
                    f"Role '{role_name}' not found while creating app client '{client_data.client_name}'."
                )
    ...
    try:
        ...
        await db.rollback()
        logger.error(
            f"Database integrity error creating app client '{client_data.client_name}': {e}"
        )
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error creating app client '{client_data.client_name}': {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    return AppClientCreatedResponse(
        ...
        allowed_callback_urls=(
            new_client.allowed_callback_urls if new_client.allowed_callback_urls else []
        ),
        assigned_roles=(
            [role.name for role in new_client.roles] if new_client.roles else []
        ),
    ...
    for client in clients:
        ...
        client_list.append(
            AppClientResponse(
                ...
                allowed_callback_urls=(
                    client.allowed_callback_urls if client.allowed_callback_urls else []
                ),
                assigned_roles=(
                    [role.name for role in client.roles] if client.roles else []
                ),
...
async def update_app_client(
    ...
    if (
        client_data.client_name is not None
        and client_data.client_name != client.client_name
    ):
        existing_client_stmt = select(AppClient).where(
            AppClient.client_name == client_data.client_name
        )
        ...
        if result.scalars().first():
            logger.warning(
                f"App client name '{client_data.client_name}' already exists."
            )
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"App client with name '{client_data.client_name}' already exists.",
    ...
    if update_data:
        try:
            ...
            await db.rollback()
            logger.error(
                f"Database integrity error updating app client '{client.client_name}': {e}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            await db.rollback()
            logger.error(
                f"Unexpected error updating app client '{client.client_name}': {e}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_app_client(
    ...
    try:
        ...
        await db.rollback()
        logger.error(
            f"Error deleting app client '{client_name}' with ID {client_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
