...
from auth_service.schemas.user_role_schemas import UserRoleAssign, UserRoleResponse, UserRoleListResponse
...
async def assign_role_to_user(
    ...
    logger.info(f"Admin user attempting to assign role {role_assignment.role_id} to user {user_id}")
...
async def list_user_roles(
    ...
    query = select(UserRole).where(UserRole.user_id == user_id).options(selectinload(UserRole.role))
    ...
    return UserRoleListResponse(items=user_role_responses, count=len(user_role_responses))
