...
from auth_service.db import get_db, AsyncSessionLocal
...
def parse_connection_url(url):
    ...
    pgbouncer_enabled = 'pgbouncer' in query_params and query_params['pgbouncer'][0] == 'true'
...
async def test_direct_connection():
    ...
    try:
        ...
        if conn_params['pgbouncer_enabled']:
            conn_kwargs["options"] = r"-c standard_conforming_strings=on -c client_encoding=utf8 -c plan_cache_mode=force_custom_plan -c statement_timeout=0 -c default_transaction_isolation=read\ committed"
        ...
        async with conn.cursor() as cur:
            ...
            for row in params:
                ...
                if any(kw in param_name for kw in ['conn', 'pool', 'timeout', 'statement', 'idle']):
...
async def test_sqlalchemy_connection():
    ...
    try:
        ...
        for row in rows:
            print(f"  {row['Database']} ({row['Size']}) - {row['Connections']} connections")
...
async def test_supabase_api():
    ...
    try:
        ...
        return response.status_code < 500  # Consider it successful if not a server error
    except Exception as e:
