# orchestrator_service/scripts/manage_db.py
...
async def bootstrap_service():
    logger.info("Running service data bootstrap for orchestrator_service...")
...
async def recreate_environment(db_params: Dict):
    ...
    """
    logger.info("--- Recreating environment for Orchestrator Service ---")
    ...
    run_command(
        f"psql -h {db_params['host']} -p {db_params['port']} -U {db_params['user']} -d {db_params['dbname']} -c '\\dt orchestrator_service_data.*'"
    )
