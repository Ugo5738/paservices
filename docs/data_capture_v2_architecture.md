...
|---|---|---|---|
| Mint authority | One SuperID flows through a normal pass | Chunks 1 + 7 applied: `Auth + Super ID Sub-Workflow` fails on missing/invalid `super_id` (no longer mints); `WF DC 1 Main` passes its `super_id` through to `WF DC B AIF`. `WF DC B2 AIF` now receives a caller-supplied super_id (reference) and mints a fresh operating super_id only for the iterative second pass, writing a link record from the new super_id to the reference. Upstream callers (Orchestrator V2, pa_mcp) must mint at scope entry — verify before deploying. | `n8n_workflows/Auth + Super ID Sub-Workflow.json`; `n8n_workflows/WF DC 1 Main - Data Capture.json:124`; `n8n_workflows/WF DC B2 AIF - AI Fetcher Multishot.json` |
