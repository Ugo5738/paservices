...
|---|---|---|---|
| Mint authority | One SuperID flows through a normal pass | Chunks 1 + 7 + 9 applied: `Auth + Super ID Sub-Workflow` fails on missing/invalid `super_id` (no longer mints); `WF DC 1 Main` passes its `super_id` through; `WF DC B2 AIF` receives a reference super_id and mints a fresh operating super_id only on its second pass (writing a link record); `SuperSami Trigger Workflow` validates a caller-supplied super_id and mints only when called as the scope entry with no super_id in the body. `Orchestrator V2 (Sequential Dependencies)` deleted (chunk 10) — `pa_mcp` now prefers `N8N_ORCHESTRATOR_V3_URL`. | `n8n_workflows/Auth + Super ID Sub-Workflow.json`; `n8n_workflows/WF DC 1 Main - Data Capture.json:124`; `n8n_workflows/WF DC B2 AIF - AI Fetcher Multishot.json`; `n8n_workflows/SuperSami Trigger Workflow.json`; `pa_mcp/src/pa_mcp/tools/n8n_service_tools.py:230` |
| Single-use check (fetcher_runs) | Each service enforces `super_id` uniqueness in its own datastore and rejects duplicates | **Chunk 4 applied.** `data_capture.fetcher_runs` now has `UNIQUE(super_id)` (migration `961cbac71cd5`). Existing NULL super_ids continue to coexist (Postgres treats multiple NULLs as distinct); chunk 5 will backfill and tighten to NOT NULL alongside the fetcher_runs schema rework. | `data_capture_service/alembic/versions/961cbac71cd5_add_unique_super_id_to_fetcher_runs.py`; `data_capture_service/src/data_capture_service/models/fetcher_run.py` |
| Progressive-state aggregation (pa_mcp, workflow_status) | Either (a) per-service single-use enforced at write time, or (b) progressive state expressed as append-only events with a "latest" view | **Pre-existing pattern, re-classified after chunk 4 review (2026-05-15).** `pa_mcp.analysis_result_crud.upsert_analysis_result`, `data_capture_rightmove_service.workflow_status_crud.upsert_status`, and `floorplan_service.workflow_status_crud.upsert_status` are upserts representing progressive state of a *single* service's use of a SuperID, not multiple uses. The aggregation is needed because multiple downstream services (data_capture, floorplan, image_condition) contribute partial data to one analysis run for `pa_mcp`, and workflow_status tracks state-machine progress for one analysis. These upserts are **not strict principles violations** (the principles forbid mutation of SuperID/activity/link records — service-internal state tables are out of scope of that rule). Full hygiene refactor (event sourcing with append-only `*_events` tables + latest-state views) is queued as an optional later chunk. | `pa_mcp/src/pa_mcp/crud/analysis_result_crud.py`; `data_capture_rightmove_service/src/data_capture_rightmove_service/crud/workflow_status_crud.py`; `floorplan_service/src/floorplan_service/crud/workflow_status_crud.py` |
...
| Immutability (SuperID Metadata records) | No UPDATE on SuperID, activity, or link records | **Chunk 2 applied** — both `activity_records` and `link_records` enforce immutability via a `BEFORE UPDATE OR DELETE` Postgres trigger (`prevent_mutation_on_superid_metadata_store`). UPDATE/DELETE on the `super_ids` table is captured by a separate audit trigger. Service-internal tables (`analysis_result`, `workflow_status`, `fetcher_runs.status`) are out of scope of this rule — see "Progressive-state aggregation" row above for their handling. The `fetcher_runs.status` lifecycle (`draft → final → superseded`) is dropped entirely in chunk 5. | `super_id_service/alembic/versions/674e901b2bd1_add_superid_metadata_store.py`; `super_id_service/db/schema.sql:28-67`; chunk 5 removes `fetcher_runs.status` lifecycle |
...
- **Chunk 8 applied: V2 promotes to canonical via an explicit primitive.** `POST /fetchers/promote-to-canonical` (keyed by `super_id`) reads the gate-passing fetcher_runs row, re-checks the gate (completeness ≥ threshold, no critical missing fields, no error), and inserts a `canonical_property_snapshots` row plus `canonical_media` rows. The endpoint is append-only — `UNIQUE(super_id)` on `canonical_property_snapshots` (migration `b75647262ca4`) rejects re-promotion with 409, matching the chunk-4 pattern. Consumers continue to read from `canonical_property_snapshots`. The V1 `_store_canonical` path is untouched and still works for V1 capture flows. n8n workflows (`WF DC A CF`, `WF DC B2 AIF`) need a "Call /promote-to-canonical" HTTP node added after their Validate-passed branches — local n8n change, not committed (n8n_workflows/ is gitignored).
- **`pa_mcp` and `floorplan_service` upsert by `super_id`** are NOT strict principles violations (chunk 4 review, 2026-05-15). They represent progressive state of a *single* service's use of the SuperID — multiple downstream-service callbacks contributing to one analysis run (`pa_mcp`) and state-machine progression for one workflow (`workflow_status`). The principles' immutability rule applies to SuperID/activity/link records, not service-internal state tables. Fuller hygiene refactor (append-only `*_events` tables + latest-state views) is queued as an optional later chunk.
...
> **Current code state.** Chunks 1 + 7 applied. `WF DC 1 Main` passes its `super_id` through to `WF DC B AIF`. The `Auth + Super ID Sub-Workflow` no longer mints when `super_id` is missing — it stops with an error. `WF DC B2 AIF` has been rewritten (chunk 7) to receive the caller's super_id (reference), mint a fresh operating super_id for the second pass only, write a link record between them, and write an optional supersession link if Pass 2 beats Pass 1.
...
| 2 | Single-use UNIQUE on `fetcher_runs` | **Done (chunk 4)** | `UNIQUE(super_id)` added via migration `961cbac71cd5`. Re-classification of pa_mcp / workflow_status upserts as "progressive state aggregation, not principles violations" documented in section 2. Event-sourcing refactor for those tables is optional follow-up. |
...
| 6 | V2 primitives don't write to `canonical_property_snapshots`; consumers reading canonical see no V2 data | **Done (chunk 8)** | `POST /fetchers/promote-to-canonical` (keyed by super_id) + `UNIQUE(super_id)` on canonical_property_snapshots (migration `b75647262ca4`). n8n workflows need the new HTTP node added at deploy time. |
| 7 | WF DC 1 Main calls WF DC B AIF directly, not WF DC B2 AIF; iterative logic is dormant | Medium | **Code resolved (chunk 7) — WF DC B2 AIF rewritten with reference SuperID + mint-and-link pattern.** Activation (wiring WF DC 1 Main to call WF DC B2 AIF when validation fails) still needs explicit decision. |
...
| WF DC B2 AIF — iterative AI fetcher using reference SuperID convention | `n8n_workflows/WF DC B2 AIF - AI Fetcher Multishot.json` | TARGET — chunk 7 applied. Receives caller-supplied super_id (reference), mints fresh operating super_id for Pass 2, writes iteration link + optional supersession link via SuperID Metadata store. |
...
| `pa_mcp.upsert_analysis_result` (ON CONFLICT DO UPDATE) | `pa_mcp/src/pa_mcp/crud/analysis_result_crud.py` | OK — progressive aggregation of multiple downstream callbacks for one analysis run (not a strict principles violation). Event-sourcing refactor is an optional later chunk. |
| `floorplan_service.upsert_status` / `data_capture_rightmove_service.upsert_status` | `floorplan_service/.../crud/workflow_status_crud.py`; `data_capture_rightmove_service/.../crud/workflow_status_crud.py` | OK — progressive state-machine of one workflow execution (not a strict principles violation). Event-sourcing refactor is an optional later chunk. |
...
| Per-service single-use check (`UNIQUE` on `super_id`) | `data_capture_service/.../models/fetcher_run.py` (`uq_fetcher_runs_super_id`, chunk 4); `pa_mcp/src/pa_mcp/models/analysis_result.py:15` (PK, intentional progressive-aggregation table); `data_capture_rightmove_service/.../models/workflow_status.py:22-24` and `floorplan_service/.../models/workflow_status.py:22-24` (`UniqueConstraint("super_id", "context")`, intentional state-machine table) | TARGET — chunk 4 applied to fetcher_runs. The other tables aggregate progressive state of one use (re-classified after chunk 4 review). |
