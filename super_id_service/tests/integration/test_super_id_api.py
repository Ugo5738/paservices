...
from super_id_service.schemas.super_id import SuperIDResponse
...
def test_generate_single_super_id(
    mock_supabase_client, client_with_permissions, mock_auth_header
):
    ...
    response = client_with_permissions.post(
        "/super_ids", json={"count": 1}, headers=mock_auth_header
    )
...
def test_generate_multiple_super_ids(
    mock_supabase_client, client_with_permissions, mock_auth_header
):
    ...
    count = 3
    response = client_with_permissions.post(
        "/super_ids", json={"count": count}, headers=mock_auth_header
    )
...
def test_generate_super_id_with_metadata(
    mock_supabase_client, client_with_permissions, mock_auth_header
):
    ...
    response = client_with_permissions.post(
        "/super_ids", json={"count": 1, "metadata": metadata}, headers=mock_auth_header
    )
...
def test_unauthenticated_request(client_unauthenticated):
    ...
    # After allowing unauthenticated creation, verify it now succeeds with 201
